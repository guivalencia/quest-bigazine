...
<html lang="pt-br">
	...
	<body>
		...
		$html .= '<td><b>Conheceu</b></td>';
		$html .= '<td><b>Loja</b></td>';
		$html .= '<td><b>Nome Cliente</b></td>';
		$html .= '<td><b>Telefone Cliente</b></td>';
	...
	FROM `quest`.`avalia_tb` WHERE loja like '%$filtro%' and data_criacao >= '$de' and data_criacao <='$ate' order by avalia_id desc;";
		...
		while($row_msg_contatos = mysqli_fetch_assoc($resultado_msg_contatos)){
			...
			$html .= '<td>'.$row_msg_contatos["pergunta6"].'</td>';
			$html .= '<td>'.$row_msg_contatos["loja"].'</td>';
			$html .= '<td>'.$row_msg_contatos["nome"].'</td>';
			$html .= '<td>'.$row_msg_contatos["telefone"].'</td>';
			$html .= '<td>'.$row_msg_contatos["comentario"].'</td>';
			$html .= '<td>'.$row_msg_contatos["comentario1"].'</td>';
